...
% Other
doorway_distance = 7;   % Distance from the starting point ([0, 0, 0])
...
l_fr_up_z = repmat(doorway_distance, size(l_fr_up_x));
...
l_fr_rt_z = repmat(doorway_distance, size(l_fr_rt_y));
...
l_rt_up_z = 0:dot_resolution:doorway_distance;
...
dr_up_z   = repmat(doorway_distance, size(dr_up_x));
...
dr_rt_z   = repmat(doorway_distance, size(dr_rt_y));
...
figure;
scatter3(outline_dots_pos(:, 1), outline_dots_pos(:, 3), outline_dots_pos(:, 2), 1, 'filled')
...
% If no 'Outline_positions.csv' in the folder and/or you have gone through the process above
dotPosition = outline_dots_pos;

% If 'Outline_positions.csv' already exists, can start here
% dotPosition = readtable('Outline_positions.csv');
...
% Remove those sampled dots that are behind the viewer
