...
<p align="center">
  ...
  For a sampled dot, any dot with a distance shorter than a closeness threshold (e.g., 0.2&deg;![closeness](https://latex.codecogs.com/gif.latex?\times\sqrt{2}=) 0.28&deg;) is considered as overlapping with the dot on the image plane, forming a potential pair of motion parallax dots. However, if the two dots are both from the target or on the same depth, they will not be considered as a valid motion parallax dots. 
